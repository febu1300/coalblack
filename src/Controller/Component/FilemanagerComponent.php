...
class FilemanagerComponent extends Component{
    public function doUpload($toupload){
      ...
      public function doUploadMultiple($toupload){
      //  $imagecache = new ImageCache\ImageCache();
        ...
        //$target_dir = $toupload->photo_dir;
        $target_file = $tempdir_main . basename($_FILES["photo"]["name"][0]);

        move_uploaded_file($_FILES["photo"]["tmp_name"][0], $target_file);
      ...
      public function doUploadPic($toupload,$theDir,$k){

      // pr($toupload->photo_dir);die();		$imagecache = new ImageCache\ImageCache();

        $desired_width=75;
        $tempdir_main=$theDir.'/main/';
        $tempdir_thumb=$theDir.'/thumb/';
          if (!file_exists($tempdir_main)) {
            mkdir($tempdir_main, 0777, true);
            mkdir($tempdir_thumb,0777,true);		
        }
if($k>0){
        //$target_dir = $toupload->photo_dir;
        $target_file = $tempdir_main . basename($_FILES["photo"]["name"][$k]);

        move_uploaded_file($_FILES["photo"]["tmp_name"][$k], $target_file);
        
        $source_image=imagecreatefromjpeg($tempdir_main.$toupload['name']);
        ...
        $virtual_image = imagecreatetruecolor($desired_width, $desired_height);
        imagecopyresampled($virtual_image, $source_image, 0, 0, 0, 0, $desired_width, $desired_height, $width, $height);
        imagejpeg($virtual_image, $tempdir_thumb.$toupload['name']);
}
return 0;	
