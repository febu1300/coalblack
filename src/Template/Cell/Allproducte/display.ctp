...
<div class=" col-sm-3 col-md-3 col-lg-3 "><p class="bold"><strong><?= $this->Number->currency($product->price,'EUR') ?></strong></p></div>        	
                                             </div>
        			
        		</div>
        	</article>
        <?php if($product['new_in']&& !$product['sale']): ?>
        <div class="text-block-new">New</div> 
          <?php  elseif($product['sale']): ?>
        <div class="text-block">Sale</div> 
        <?php  endif; ?>
    </div>
        </div> 
      <?= $this->Form->end() ?>
