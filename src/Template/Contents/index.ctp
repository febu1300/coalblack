...
<nav class="navbar navbar-expand-lg navbar-dark bg-light" id="actions-sidebar">
    <ul class="nav nav-pills flex-column">
           ...
           <li class="nav-item"><?= $this->Html->link(__('New Content'), ['action' => 'add']) ?></li>
    </ul>
</nav>
        </div>
    ...
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                ...
                <th scope="col"><?= $this->Paginator->sort('lable') ?></th>
                <th scope="col"><?= $this->Paginator->sort('link_page') ?></th>
       ...
       <td> 
        <img src="<?='/'. h($content->photo_dir.'/thumb/'.$content->photo)?>">
           <?= $this->Html->link(__(''), 
            ['action' => 'changepic', $content->id], 
            ['class' => "glyphicon glyphicon-edit", 'data-toggle' => "modal", 'data-target' => "#changeImage"]) ?>
        </td>
                <td class="actions">
     <?= $this->Form->postLink(__(''), ['action' => 'delete', $content->id], ['confirm' => __('Are you sure you want to delete # {0}?', $content->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
